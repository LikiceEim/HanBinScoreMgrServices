...
namespace iCMS.WG.Agent.Common
{
    ...
    public class LogHelper
    {
        ...
        //记录上次压缩时间的文件名
        private static string LastCompressDateTimeFileName = "RecordLastCompressData.txt";
        //记录上次压缩文件路径
        private static string LastCompressDateTimePath = string.Empty;
        #endregion
        ...
        private static void Write(string path, string context)
        {
            ...
            LastCompressDateTimePath = path + @"\" + LastCompressDateTimeFileName;
            if (!File.Exists(LastCompressDateTimePath))
            {
                CreateRecordLastDateTime(LastCompressDateTimePath);
            }
            lock (thisLock)
            {
        ...
        static void CompressAndDeleteLog(string path)
        {
            ...
            if (days > 0)
            {
                if (DateTime.Now.Date !=  ReadLastDateTime(LastCompressDateTimePath).Date)
                {
        ...
        #region 记录最后压缩文件时间
        /// <summary>
        /// 记录最后压缩文件时间 add by masu 2016年5月4日15:10:57
        /// </summary>
        /// <param name="fileName"></param>
        private static void CreateRecordLastDateTime(string fileName)
        {
        ...
        #region 读取TXT文件内时间
        /// <summary>
        /// 读取TXT文件内时间 add by masu 2016年5月4日15:11:34
        /// </summary>
        /// <param name="fileName"></param>
        /// <returns></returns>
        private static DateTime ReadLastDateTime(string fileName)
        {
            using (StreamReader sr = new StreamReader(fileName))
            {
                ...
                {
                    CreateRecordLastDateTime(fileName);
                }
                sr.Close();
