...
namespace HanBin.Service.Common
{
    ...
    public static class CollectionsExtensions
    {
        ...
        /// 创建人：张辽阔
        /// 创建时间：2016-09-08
        /// 创建记录：把IQueryable[T]集合按指定属性排序条件进行排序
        /// </summary>
        /// <typeparam name="T">动态类型</typeparam>
        /// <param name="source">要排序的数据集</param>
        /// <param name="sortConditions">列表属性排序条件</param>
        /// <returns></returns>
        public static IOrderedQueryable<T> OrderBy<T>(this IQueryable<T> source, params PropertySortCondition[] sortConditions)
        {
            PublicHelper.CheckArgument(sortConditions, "sortConditions");
            if (!sortConditions.Any())
            {
                throw new ArgumentNullException("sortConditions", "排序不能为空");
            }
            int count = 0;
            IOrderedQueryable<T> orderSource = null;
            foreach (PropertySortCondition sortCondition in sortConditions)
            {
                if (sortCondition != null && !string.IsNullOrWhiteSpace(sortCondition.PropertyName))
                {
                    orderSource = count == 0
        ...
        /// 创建人：张辽阔
        /// 创建时间：2016-09-08
        /// 创建记录：把IOrderedQueryable[T]集合继续指定属性排序方式进行排序
        /// </summary>
        /// <typeparam name="T">动态类型</typeparam>
        /// <param name="source">要排序的数据集</param>
        /// <param name="sortConditions">列表属性排序条件</param>
        /// <returns></returns>
        public static IOrderedQueryable<T> ThenBy<T>(this IOrderedQueryable<T> source, params PropertySortCondition[] sortConditions)
        {
            PublicHelper.CheckArgument(sortConditions, "sortConditions");
            if (!sortConditions.Any())
            {
            ...
            foreach (PropertySortCondition sortCondition in sortConditions)
            {
                if (sortCondition != null && !string.IsNullOrWhiteSpace(sortCondition.PropertyName))
                {
                    orderSource = source.ThenBy(sortCondition.PropertyName, sortCondition.ListSortDirection);
                    count++;
                }
            }
            if (count == 0)
        ...
        /// 创建人：张辽阔
        /// 创建时间：2016-09-08
        /// 创建记录：从指定IQueryable[T]集合 中查询指定分页条件的子数据集
        /// </summary>
        ...
        /// <returns></returns>
        public static IQueryable<TEntity> Where<TEntity>(this IQueryable<TEntity> source, int pageIndex, int pageSize,
            out int total, params PropertySortCondition[] sortConditions) where TEntity : EntityBase
        {
            ...
            {
                orderSource = source.OrderBy(m => m.AddDate);
            }
            else
            {
                int count = 0;
            ...
            return orderSource != null
                ? orderSource.Skip((pageIndex - 1) * pageSize).Take(pageSize)
        ...
        /// <param name="sortConditions">排序条件集合</param>
        /// <returns></returns>
        public static IQueryable<TEntity> PagerOrder<TEntity>(this IQueryable<TEntity> source, int pageIndex, int pageSize,
